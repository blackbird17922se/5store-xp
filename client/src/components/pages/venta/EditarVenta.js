...
export default class EditarVenta extends Component {
    ...
    componentDidMount(){
        ...
        Axios.get(`/Rventa/detailVenta/${id}`).then((res) => {
            if(res.data.success){
                this.setState({

                    descrip: res.data.venta.descrip,
                    valUnit: res.data.venta.valUnit,
                    estado: res.data.venta.estado,
                    cant: res.data.venta.cant,
                    fecha: res.data.venta.fecha,
                    doClient: res.data.venta.doClient,
                    nomClient: res.data.venta.nomClient,
                });
    ...
    onSubmit = (e) => {
        ...
        // if(this.validar(idProd,descrip,valUnit,estado,doClient,nomClient,cant,totalV,fecha)){
            ...
            Axios.put(`/Rventa/updateVenta/${id}`, data).then((res) => {
                if(res.data.success){
        ...
        return (
            <React.Fragment>
                ...
                <div className="col-md-10 mt-3 mx-auto">
                    <h1>Editar Venta {this.state.descrip}</h1>
